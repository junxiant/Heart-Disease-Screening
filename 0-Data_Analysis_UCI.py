...
logging.basicConfig(level=logging.INFO, 
                    ...
                    handlers=[
                        logging.FileHandler("./figs/UCI_Data_Analysis.log"),
...
df = pd.read_csv(f"{data_path}/heart_disease_uci.csv")
...
for col in df.select_dtypes(include=['int64', 'float64']).columns:
    if df[col].nunique() < 20:  # Threshold for treating a numeric column as categorical
...
# 1. Data distribution for the UCI dataset
...
# plt.show()
plt.savefig('./figs/UCI_Heart_Disease_Data_Distribution_Before_Split.png')
...
categorical_vars = ['sex', 'cp', 'fbs', 'restecg', 'exang', 'slope', 'thal', 'ca', 'num']
...
plt.tight_layout()
plt.savefig('./figs/UCI_Categorical_Analysis.png')
...
# plt.show()
plt.savefig('./figs/UCI_Age_Analysis.png')
...
plt.ylabel('Count')
# plt.show()
plt.savefig('./figs/UCI_Chol_Analysis.png')
...
# Columns to drop
columns_to_drop = df.isnull().sum()[df.isnull().sum()>0].index.tolist()
# Drop
...
# Do corr with label
correlation_with_target = train_data.drop('num', axis=1).apply(lambda x: x.corr(train_data['num'])).abs().sort_values(ascending=False)
...
plt.tight_layout()
# plt.show()
plt.savefig('./figs/UCI_Correlation_Bar.png')
...
plt.tight_layout()
# plt.show()
plt.savefig('./figs/UCI_Correlation_Matrix_Heatmap.png')
...
plt.tight_layout()
# plt.show()
plt.savefig('./figs/UCI_Feature_Importance_RF.png')
